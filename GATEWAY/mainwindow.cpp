...
//    FileData file(DATA_PATH);
   ...
   // WriteDatatoLogfile(tmp);
    ...
    double dis = rand() % 50 + 120;
	double dismin = rand() % 20 + 100;
    double H2S = rand() % 30 + 10;
    double ND = rand() % 10 + 1;
    bool ND1 = "True";
	double ph = rand() % 14 + 1;
	double maxot = rand() % 50 + 100;
	double minot = rand() % 50 + 70;
	double maxop = rand() % 50 + 100;
	double minop = rand() % 50 + 70;
	double critical = rand() % 50 +120;
	double hydrogen = rand() % 50 + 100;
    double cladding = rand() % 50 + 120;
	double h2scontent = rand() % 50 + 120;
    sendMqttData(mac1, eq, "equipment_volume");
    sendMqttData(mac1, dis, "distance_to_ground_water");
    sendMqttData(mac1, maxot, "max.operating_temperature");
    sendMqttData(mac1, minot, "min.operating_temperature");
    sendMqttData(mac1, maxop, "max.operating_pressure");
    sendMqttData(mac1, minop, "min.operating_pressure");
	sendMqttData(mac1, critical, "critical_exposure_temperature");
	sendMqttData(mac1, hydrogen, "operating_hydrogen_partial_pressure");
	sendMqttData(mac1, cladding, "cladding_thinkness");
	sendMqttData(mac1, h2scontent, "h2s_content_in_water");
    receivedFlag = true;
//    emit readySend();
}
    ...
    QFile file("./WSAN/AIDustDensity.txt");
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text | QIODevice::ReadWrite)) {
            ...
            while (!stream.atEnd()) {
                ...
                console->insertPlainText(tmp);
                double eq = rand() % 50 + 100;
                double dis = rand() % 50 + 100;
                double H2S = rand() % 30 + 10;
                double ND = rand() % 10 + 1;
                bool ND1 = "True";
				double ph = rand() % 14 + 1;
				double fluid = rand() % 30 + 10;
				double naoh =  rand() % 10+1;
				double ND2 = rand() % 10 + 1;
				double ND3 = rand() % 10 + 1;
                ...
                sendMqttData(mac, H2S, "flow_rate");
                sendMqttData(mac, ph, "ph_of_water");
                //sendMqttData(mac, eq, "critical_exposure_temperature");
                //sendMqttData(mac, eq, "cladding_thinkness");
                sendMqttData(mac, fluid, "fluid_height");
                sendMqttData(mac, naoh, "naoh_concentration");
                //sendMqttData(mac, dis, "h2s_content_in_water");
                sendMqttData(mac, ND2, "min.required_thickness");
                sendMqttData(mac, ND3, "current_corrosion-rate");
                sendMqttData(mac, ND1, "presence_of_crack");
                sendMqttData(mac, ND1, "interface_at_soil_or_water");
                sendMqttData(mac, ND1, "external_coating");
                sendMqttData(mac, ND1, "internal_coating");
                sendMqttData(mac, ND1, "internal_cladding");
                sendMqttData(mac, ND1, "steamed_out_prior_to_water_flushing");
                sendMqttData(mac, ND1, "exposed_to_acid_gas_treating_amine");
                sendMqttData(mac, ND1, "environment_contains_H2S");
				sendMqttData(mac, H2S, "current_thickness");
